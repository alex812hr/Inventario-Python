...
class AuditoriaBCP:
    ...
    def cargar_y_procesar(self):
        print("📁 POR FAVOR, SUBE TU ARCHIVO EXCEL (.xlsx)")
        uploaded = files.upload()

        if not uploaded: # Check if any file was uploaded
            print("❌ No se seleccionó ningún archivo. Por favor, sube tu archivo Excel para continuar.")
        ...
        # Calculamos días solo para atendidos (EST1 == 1)
        ...
        # Filtramos ing (ingresos) para ser consistente si N_COMPRA 801 existiera aquí
        ing_filtered = ing[ing['N_COMPRA'] != 801].copy()


        # 2. Lógica de Trazabilidad (ITEM + N_COMPRA)
        ...
        # 3. Calcular métricas de tiempo con los datos filtrados
        # Solo consideramos días de atención positivos para 'oc_rapidas' para evitar errores de registro.
        ...
        # Promedio general de atención (de los atendidos y filtrados)
    ...
    def generar_visualizaciones(self):
        ...
        # Gráfico 1: Estado de Pedidos (Pie Chart)
        # Utiliza self.df_salidas que ya está filtrado
        ...
        axes[0].set_title('Eficiencia de Atención (EST1) - Excluyendo OC #801')

        # Gráfico 2: Top 10 Productos con mayor Stock
        # Utiliza self.df_stock que ya está filtrado
    ...
    def exportar_excel(self):
        ...
        # Usamos ExcelWriter para poder guardar varias hojas si quisiéramos
        with pd.ExcelWriter(nombre_file) as writer:
            self.df_stock.to_excel(writer, sheet_name='STOCK_FINAL', index=False)
            # También exportamos las métricas de tiempo
            # Asegurarse de usar self.df_salidas que ya está filtrado
        ...
        print(f"\n✅ Excel generado y descargado: {nombre_file}")
...
if app.df_stock is not None:
    print(f"\n🏆 LA ORDEN DE COMPRA MÁS RÁPIDA (excluyendo OC #801): OC #{app.reporte_operativo['mejor_oc']}")
    print(f"⏱️ Tiempo promedio de atención (excluyendo OC #801): {app.reporte_operativo['promedio_atencion']:.2f} días")
